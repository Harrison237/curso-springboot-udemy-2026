...
import com.harrison.curso.springboot.app.security.filter.JwtAuthenticationFilter;
import com.harrison.curso.springboot.app.security.filter.JwtValidationFilter;
...
public class SpringSecurityConfig {
    ...
    SecurityFilterChain filterChain(HttpSecurity http) {
        return http.authorizeHttpRequests(authz -> authz
                ...
                /*
                 * .requestMatchers(HttpMethod.POST, "/api/users").hasRole(Role.ADMIN)
                 * .requestMatchers(HttpMethod.POST, "/api/products").hasRole(Role.ADMIN)
                 * .requestMatchers(HttpMethod.GET, "/api/products",
                 * "/api/products/{id}").hasAnyRole(Role.ADMIN, Role.USER)
                 * .requestMatchers(HttpMethod.PUT, "/api/products/{id}").hasRole(Role.ADMIN)
                 * .requestMatchers(HttpMethod.DELETE, "/api/products/{id}").hasRole(Role.ADMIN)
                 */
